...
Mimir Lambda runs your function code locally inside real Docker containers - close enough as AWS Lambda does (using Firecracker micro VM).
...
Mimir supports an automatic hot-reloading mechanism when functions are deployed via S3. This follows the standard AWS behavior where S3 and Lambda interact, but is optimized for a seamless local development experience.

When a Lambda function is created using an S3 bucket and key, Mimir maintains a link between the function and its source object. Any subsequent update to that S3 object (e.g., via `s3:PutObject`) automatically triggers a reactive synchronization:

1.  **Detection**: Mimir detects the S3 update via an internal event system.
...
For the tightest inner-loop development cycle, Mimir supports a **bind-mount hot-reload** mode. Instead of packaging code into a ZIP and uploading it to S3, you point Mimir directly at a directory on your host machine. The directory is bind-mounted into `/var/task` inside the container, so every invocation runs the files as they currently exist on disk — no upload, no redeploy.
...
The `S3Key` must be an **absolute path** reachable by the Docker daemon. When Mimir runs in Docker Compose, this is the path on the Docker host (the machine running Docker), not the path inside the Mimir container.
...
2. On each invocation, Mimir starts a **fresh ephemeral container** with the host path bind-mounted at `/var/task`.
...
MIMIR_SERVICES_LAMBDA_HOT_RELOAD_ALLOWED_PATHS=/home/user/projects,/tmp
```
...
- The `S3Key` path is interpreted by the **Docker daemon**, not by Mimir. When Mimir itself runs inside Docker, the path must exist on the Docker host machine, not inside the Mimir container.
...
!!! note "Concurrency enforcement"
    ...
    in fact a per-account-per-region quota, and Mimir mirrors that by
    partitioning counters on the ARN's region segment. The pool size (default
    1000) is configurable via `mimir.services.lambda.region-concurrency-limit`
    and applies independently to each region. `PutFunctionConcurrency`
    validates that the requested value leaves at least
    `mimir.services.lambda.unreserved-concurrency-min` (default 100) available
...
These AWS Lambda operations have no handler in Mimir. Calls will return `404` or an error:
...
|---|---|---|
| `MIMIR_SERVICES_LAMBDA_ENABLED` | `true` | Enable or disable the service |
| `MIMIR_SERVICES_LAMBDA_EPHEMERAL` | `false` | Remove containers after each invocation |
| `MIMIR_SERVICES_LAMBDA_DEFAULT_MEMORY_MB` | `128` | Default function memory (MB) |
| `MIMIR_SERVICES_LAMBDA_DEFAULT_TIMEOUT_SECONDS` | `3` | Default function timeout (seconds) |
| `MIMIR_SERVICES_LAMBDA_RUNTIME_API_BASE_PORT` | `9200` | First port in the Lambda Runtime API range |
| `MIMIR_SERVICES_LAMBDA_RUNTIME_API_MAX_PORT` | `9299` | Last port in the Lambda Runtime API range |
| `MIMIR_SERVICES_LAMBDA_CODE_PATH` | `./data/lambda-code` | Directory where Lambda ZIP files are stored |
| `MIMIR_SERVICES_LAMBDA_POLL_INTERVAL_MS` | `1000` | Event-source mapping poll interval (milliseconds) |
| `MIMIR_SERVICES_LAMBDA_CONTAINER_IDLE_TIMEOUT_SECONDS` | `300` | Idle container shutdown timeout (seconds) |
| `MIMIR_SERVICES_LAMBDA_REGION_CONCURRENCY_LIMIT` | `1000` | Maximum concurrent executions per region |
| `MIMIR_SERVICES_LAMBDA_UNRESERVED_CONCURRENCY_MIN` | `100` | Minimum unreserved capacity `PutFunctionConcurrency` must leave |
| `MIMIR_SERVICES_LAMBDA_HOT_RELOAD_ENABLED` | `false` | Enable bind-mount hot-reload via `S3Bucket=hot-reload` |
| `MIMIR_SERVICES_LAMBDA_HOT_RELOAD_ALLOWED_PATHS` | *(unset)* | Comma-separated allowlist of host paths that may be bind-mounted |
...
`https://my-bucket.s3.amazonaws.com/key`. Against Mimir the same pattern becomes
`http://my-bucket.localhost.mimir.local:4566/key`.

When Mimir runs **inside Docker**, Lambda containers are on the same Docker
network. Docker's embedded DNS resolves the exact alias `localhost.mimir.local`
correctly, but has no wildcard support — `my-bucket.localhost.mimir.local`
falls through to public DNS and resolves to the wrong IP, causing the Lambda
invocation to time out.

**Mimir solves this automatically** by running an embedded DNS server (UDP/53)
on its container IP. All Lambda containers launched by Mimir are configured to
use it as their DNS resolver. The embedded DNS server:

- Resolves `*.localhost.mimir.local` → Mimir's Docker network IP
...
`CAP_NET_BIND_SERVICE` in their default capability set, so Mimir (running as a
...
!!! note "Resolving public hostnames from Lambda"
    ...
    `business-api.tiktok.com`) resolves it through Mimir's embedded DNS. As a
    safety net, Mimir also appends configurable public resolvers (default
    ...
    ```bash
    MIMIR_DNS_CONTAINER_FALLBACK_SERVERS=1.1.1.1,1.0.0.1   # use different resolvers
    MIMIR_DNS_CONTAINER_FALLBACK_ENABLED=false             # inject only Mimir's DNS
    ```

!!! tip "Docker Compose service names"
    If Mimir runs as a Docker Compose service, set `MIMIR_HOSTNAME` to the
    service name, for example `MIMIR_HOSTNAME=mimir`. When no explicit Lambda
    Docker network is configured, Mimir automatically attaches Lambda
    containers to the current Compose network. Mimir then injects
    `AWS_ENDPOINT_URL=http://mimir:4566` into Lambda containers and returns
    SQS `QueueUrl` values with the same reachable host.

    This avoids function-side rewrites from `localhost` or `localhost.mimir.local`
    to `mimir`, and keeps normal AWS SDK clients pointed at the Docker DNS name
    that the Lambda container can resolve.

!!! note "Path-style as a workaround"
    If you cannot use virtual-hosted-style (e.g. Mimir is running natively on
...
hardcoded, add the LocalStack suffix to Mimir's DNS resolver so it resolves to
Mimir's IP without any function-side changes:

Via environment variable — use a comma-separated list for multiple suffixes:

```bash
# Single suffix
MIMIR_DNS_EXTRA_SUFFIXES=localhost.localstack.cloud

# Multiple suffixes
MIMIR_DNS_EXTRA_SUFFIXES=localhost.localstack.cloud,localhost.example.internal
```

### Real AWS Credentials

By default, Mimir injects placeholder credentials (`test`/`test`/`test`) into Lambda containers. This is sufficient when all SDK calls target Mimir's emulated services.
...
services:
  mimir:
    ...
    environment:
      MIMIR_SERVICES_LAMBDA_AWS_CONFIG_PATH: /Users/me/.aws
    volumes:
      - /var/run/docker.sock:/var/run/docker.sock
```
...
When unset (default), Mimir reads credentials from its own environment and falls back to `test`/`test`/`test`.

!!! tip "Routing specific services to real AWS"
    To keep some services on Mimir while others hit real AWS, clear the global endpoint and set service-specific overrides in your function's `--environment`:

    ```
    AWS_ENDPOINT_URL=                                          # clear Mimir's global endpoint
    AWS_ENDPOINT_URL_SES=http://localhost.mimir.local:4566       # SES stays on Mimir
    AWS_ENDPOINT_URL_CLOUDWATCHLOGS=http://localhost.mimir.local:4566  # CloudWatch stays on Mimir
    ```
...
!!! note "Credential passthrough without mounting"
    If you don't need the full `~/.aws` directory (e.g., you only have static credentials), you can pass them to Mimir's environment directly. When `aws-config-path` is unset, Mimir forwards its own `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, and `AWS_SESSION_TOKEN` env vars into Lambda containers:
...
Any runtime that has an official AWS Lambda container image works with Mimir (e.g. `nodejs22.x`, `python3.13`, `java21`, `go1.x`, `provided.al2023`).
