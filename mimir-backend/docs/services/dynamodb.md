...
|---|---|---|
| `MIMIR_SERVICES_DYNAMODB_ENABLED` | `true` | Enable or disable the service |
| `MIMIR_STORAGE_SERVICES_DYNAMODB_MODE` | *(global default)* | Storage mode override for DynamoDB (`memory`, `persistent`, `hybrid`, `wal`) |
| `MIMIR_STORAGE_SERVICES_DYNAMODB_FLUSH_INTERVAL_MS` | `5000` | Flush interval for `hybrid`/`wal` storage modes (milliseconds) |
