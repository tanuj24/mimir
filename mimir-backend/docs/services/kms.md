...
|---|---|---|
| `MIMIR_SERVICES_KMS_ENABLED` | `true` | Enable or disable the service |
...
aws kms generate-data-key \
  ...
  --endpoint-url $AWS_ENDPOINT_URL
```
`CreateKey` also accepts a reserved creation-time tag key, `mimir:override-id`, when tests need a deterministic `KeyId`. Mimir uses the tag value as the created key id, strips the reserved tag from stored resource tags, and rejects attempts to add `mimir:*` tags later via `TagResource`.
