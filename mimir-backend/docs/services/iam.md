...
Mimir seeds a catalog of commonly-used AWS managed policies at startup. These are attachable immediately without any setup:
...
All seeded policies use a permissive wildcard document since Mimir does not enforce IAM policy evaluation by default.

## IAM Enforcement Mode

By default Mimir accepts any credentials without enforcing IAM policies — all requests are allowed through regardless of what policies are attached to the calling identity. This preserves backward compatibility and keeps the default setup frictionless.
...
```bash
MIMIR_SERVICES_IAM_ENFORCEMENT_ENABLED=true
```
...
|---|---|---|
| `MIMIR_SERVICES_IAM_ENABLED` | `true` | Enable or disable the service |
| `MIMIR_SERVICES_IAM_ENFORCEMENT_ENABLED` | `false` | Enforce IAM policies on all inbound requests |
