...
**RestoreObject** is accepted but stubbed: Mimir validates the request and returns `202 Accepted`, but no restore state machine runs.

## S3 Select

`SelectObjectContent` runs SQL queries directly against S3 objects without downloading the entire file. Mimir supports CSV, JSON Lines, JSON arrays, and Parquet inputs. The SQL dialect follows [AWS S3 Select SQL reference](https://docs.aws.amazon.com/AmazonS3/latest/userguide/s3-select-sql-reference.html).

### Execution modes

Mimir chooses the execution engine automatically based on the input format and whether the [mimir-duck](https://hub.docker.com/r/mimir/mimir-duck) sidecar is running:

| Condition | Engine | Notes |
|---|---|---|
| Input is Parquet | mimir-duck (required) | DuckDB's `read_parquet` — sidecar must be available |
| Input is CSV with `FileHeaderInfo=USE` and mimir-duck is running | mimir-duck | Full DuckDB SQL: all operators, LIKE, BETWEEN, IN, IS NULL, AND/OR/NOT |
| Input is JSON and mimir-duck is running | mimir-duck | `read_json_auto` — supports JSON Lines and JSON arrays |
| Input is CSV with `FileHeaderInfo=NONE` or `IGNORE`, or mimir-duck is not running | Java evaluator | Supports SELECT *, column projection, simple WHERE with =, !=, <, >, <=, >=, LIKE, BETWEEN, IN, IS NULL, AND/OR/NOT, LIMIT |

The mimir-duck sidecar starts lazily on the **first Athena query**. Until then, `isAvailable()` returns false and S3 Select falls back to the Java evaluator for CSV and JSON. Once the sidecar is running, subsequent S3 Select calls route through DuckDB automatically.

If mimir-duck is not running and the object is Parquet, S3 Select returns an error — Parquet decoding requires DuckDB.
...
When using the Java evaluator (no mimir-duck, or CSV with `FileHeaderInfo=NONE/IGNORE`):
...
When `MIMIR_SERVICES_ATHENA_MOCK=true` is set, Athena queries are stubbed but mimir-duck does **not** start. In that configuration, S3 Select uses the Java evaluator for CSV and JSON. Parquet queries will fail unless `MIMIR_SERVICES_DUCK_URL` points to an already-running mimir-duck instance.

## Not Implemented

These AWS S3 features have no handler in Mimir. Calls will return an error (typically `404` or `NoSuchBucket`-style):
...
|---|---|---|
| `MIMIR_SERVICES_S3_ENABLED` | `true` | Enable or disable the service |
| `MIMIR_SERVICES_S3_DEFAULT_PRESIGN_EXPIRY_SECONDS` | `3600` | Default pre-signed URL expiry (1 hour) |
| `MIMIR_AUTH_PRESIGN_SECRET` | `local-emulator-secret` | Secret used to sign pre-signed URLs |
...
Mimir supports both **path-style** and **virtual-hosted style** S3 addressing.
...
```
http://my-bucket.s3.localhost.mimir.local:4566/my-key
```

Mimir supports this natively — no `forcePathStyle` needed. The following wildcard DNS domains resolve to `127.0.0.1` via public DNS, so virtual-hosted requests reach Mimir on the host machine automatically:

| Domain pattern | Resolves to |
|---|---|
| `*.localhost.mimir.local` | `127.0.0.1` |
| `*.s3.localhost.mimir.local` | `127.0.0.1` |
| `*.localhost.localstack.cloud` | `127.0.0.1` |
| `*.s3.localhost.localstack.cloud` | `127.0.0.1` |

Plain `http://localhost:4566` also works without `forcePathStyle` — the SDK sends `Host: my-bucket.localhost:4566` and Mimir's filter extracts the bucket name from that header.
...
=== "Java"
    ...
    ```java
    S3Client s3 = S3Client.builder()
        .endpointOverride(URI.create("http://s3.localhost.mimir.local:4566"))
        .region(Region.US_EAST_1)
        .credentialsProvider(StaticCredentialsProvider.create(
            AwsBasicCredentials.create("test", "test")))
        .build();
    // SDK sends requests to: my-bucket.s3.localhost.mimir.local:4566
    ```
...
=== "Node.js"
    ...
    ```javascript
    const s3 = new S3Client({
      endpoint: "http://s3.localhost.mimir.local:4566",
...
=== "Python"
    ...
    ```python
    s3 = boto3.client("s3",
        endpoint_url="http://s3.localhost.mimir.local:4566")
    # SDK sends requests to: my-bucket.s3.localhost.mimir.local:4566
    ```

#### Virtual-Hosted Style Inside Docker

Inside Docker containers, `127.0.0.1` resolves to the container itself — not Mimir. Mimir's **embedded DNS server** handles this automatically: it resolves `*.localhost.mimir.local`, `*.s3.localhost.mimir.local`, `*.localhost.localstack.cloud`, and `*.s3.localhost.localstack.cloud` to Mimir's container IP on the Docker network.

To use virtual-hosted style from a test container, point its DNS at Mimir and set the endpoint to Mimir's service hostname:

```bash
MIMIR_IP=$(docker inspect -f '{{.NetworkSettings.Networks.mimir_default.IPAddress}}' mimir)
...
With `MIMIR_HOSTNAME=mimir` set on the Mimir container (default in the provided `docker-compose.yml`), the embedded DNS resolves `my-bucket.mimir` to Mimir's IP, and `S3VirtualHostFilter` extracts the bucket name from the Host header.

## Object Attribute Notes

Mimir now persists and returns the following object attribute state on S3 object APIs:
...
- cross-account canned ACL variants collapse to the emulator's single synthetic owner where Mimir does not model a distinct second principal
- `aws-exec-read` is accepted for compatibility, but Mimir does not yet model a distinct EC2 bundle-reader grantee in `GetObjectAcl`
