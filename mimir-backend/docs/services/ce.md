...
Mimir synthesizes Cost Explorer responses from its own resource state,
multiplied by the bundled AWS Pricing snapshot served by the
[Pricing service](pricing.md). Costs reflect what's running in Mimir right now,
...
Each Mimir service that wants to participate in cost reporting ships a
...
| Other Mimir services (DDB, SQS, SNS, …) | catalog only, zero quantity | `UnpricedServicesEnumerator` |
...
| `Credit` | When `MIMIR_SERVICES_CE_CREDIT_USD_MONTHLY > 0` (see below) |
| `Tax` / `Refund` / `DiscountedUsage` / `SavingsPlan*` | Reserved for future PRs; not currently emitted |

### Synthetic credit injection

Set `MIMIR_SERVICES_CE_CREDIT_USD_MONTHLY` (default `0.0`) to emit a monthly
...
|---|---|---|
| `MIMIR_SERVICES_CE_ENABLED` | `true` | Enable or disable the service |
| `MIMIR_SERVICES_CE_CREDIT_USD_MONTHLY` | `0.0` | Synthetic monthly credit, applied as a `Credit` `RECORD_TYPE` row |
