...
Mimir emulates Amazon Athena with **real SQL execution** powered by a [mimir-duck](https://hub.docker.com/r/mimir/mimir-duck) sidecar container running DuckDB. When a query is submitted, Mimir spins up the sidecar on first use, injects `CREATE OR REPLACE VIEW` statements for each Glue-registered table pointing to S3 data, then executes the SQL and stores results as CSV in S3.
...
1. **Lazy sidecar start**: On the first `StartQueryExecution` call, Mimir checks for a local `mimir/mimir-duck:latest` image and starts the container. Subsequent queries reuse the running container.
2. **Glue DDL injection**: Mimir reads all Glue tables for the target database and generates `CREATE OR REPLACE VIEW` statements mapping each table name to its S3 location via DuckDB's `read_parquet`, `read_json_auto`, or `read_csv_auto` functions — chosen based on the table's `InputFormat` or SerDe serialization library.
...
|---|---|---|
| `MIMIR_SERVICES_ATHENA_MOCK` | `false` | Set to `true` to disable DuckDB execution — queries immediately succeed with empty results |
| `MIMIR_SERVICES_DUCK_DEFAULT_IMAGE` | `mimir/mimir-duck:latest` | DuckDB sidecar image pulled on first use |
| `MIMIR_SERVICES_DUCK_URL` | *(unset)* | Point to an existing mimir-duck instance and skip container management |
...
Set `MIMIR_SERVICES_ATHENA_MOCK=true` to skip DuckDB entirely for Athena. In this mode queries transition to `SUCCEEDED` immediately with an empty result set — useful for unit tests that only exercise the Athena state machine, not the query results.

When mock mode is enabled the sidecar does **not** start. S3 Select will use the Java evaluator for CSV and JSON. Parquet queries will fail unless `MIMIR_SERVICES_DUCK_URL` points to an already-running mimir-duck instance.
