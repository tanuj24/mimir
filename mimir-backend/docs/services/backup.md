...
```bash
MIMIR_SERVICES_BACKUP_JOB_COMPLETION_DELAY_SECONDS=3
```
...
|---|---|---|---|
| `mimir.services.backup.enabled` | `MIMIR_SERVICES_BACKUP_ENABLED` | `true` | Enable / disable the service |
| `mimir.services.backup.job-completion-delay-seconds` | `MIMIR_SERVICES_BACKUP_JOB_COMPLETION_DELAY_SECONDS` | `3` | Seconds from job start until `COMPLETED` |
