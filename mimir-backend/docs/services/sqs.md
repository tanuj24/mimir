...
For test assertions and debugging, Mimir exposes a LocalStack-compatible endpoint that lets you peek at queue contents without consuming messages:
...
|---|---|---|
| `MIMIR_SERVICES_SQS_ENABLED` | `true` | Enable or disable the service |
| `MIMIR_SERVICES_SQS_DEFAULT_VISIBILITY_TIMEOUT` | `30` | Default message visibility timeout (seconds) |
| `MIMIR_SERVICES_SQS_MAX_MESSAGE_SIZE` | `262144` | Maximum message size in bytes (256 KB) |
| `MIMIR_SERVICES_SQS_CLEAR_FIFO_DEDUPLICATION_CACHE_ON_PURGE` | `false` | When `true`, `PurgeQueue` also clears the FIFO deduplication cache for the queue and any SNS FIFO topics subscribed to it |
