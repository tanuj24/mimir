...
This guide gets Mimir running and verifies that AWS CLI commands work against it in under five minutes.
...
Mimir accepts any dummy credentials — no real AWS account needed.
...
echo "hello mimir" | aws s3 cp - s3://my-bucket/hello.txt --endpoint-url $AWS_ENDPOINT_URL
...
Point your AWS SDK to Mimir the same way:
...
Mimir emulates ECR with a real OCI registry behind it, so the stock `docker` client works against repositories you create through the AWS CLI. No daemon configuration needed — Mimir returns repository URIs that resolve to loopback, which `docker` auto-trusts as insecure.

```bash
# Create the repository (lazy-starts the backing registry container)
aws ecr create-repository --repository-name mimir-it/app --endpoint-url $AWS_ENDPOINT
...
docker tag  alpine:3.19 000000000000.dkr.ecr.us-east-1.localhost:5000/mimir-it/app:v1
docker push             000000000000.dkr.ecr.us-east-1.localhost:5000/mimir-it/app:v1

# Pull from a clean local image store
docker rmi  000000000000.dkr.ecr.us-east-1.localhost:5000/mimir-it/app:v1
docker pull 000000000000.dkr.ecr.us-east-1.localhost:5000/mimir-it/app:v1
```
...
When Mimir runs **natively on a Linux host** (not Docker Desktop), Lambda function containers reach Mimir's Runtime API server via the docker bridge gateway. On Ubuntu / Pop!_OS / Debian boxes with **UFW enabled**, the default `INPUT DROP` policy silently drops these packets and Lambda invocations time out with `Function.TimedOut`. This affects every Lambda packaging type — Zip *and* image-backed functions deployed via emulated ECR.

**One-time fix**, scoped to the docker bridge only (does not expose anything to the network — `docker0` is internal):

```bash
sudo ufw allow in on docker0 comment 'mimir: containers reach host'
```

If you want to scope it tighter to just the Lambda Runtime API and the ECR registry port ranges:

```bash
sudo ufw allow in on docker0 to any port 9200:9299 proto tcp comment 'mimir lambda runtime api'
sudo ufw allow in on docker0 to any port 5000:5099 proto tcp comment 'mimir ecr registry'
```

**Docker Desktop** (macOS / Windows / Linux) does not need this — it routes container → host through the Docker VM, which Mimir's `DockerHostResolver` detects automatically.

**Mimir-in-Docker** (running the published Mimir image inside a container) does not need this either — Lambda containers and Mimir share the same docker network and reach each other via container IPs.
