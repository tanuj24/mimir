...
Mimir supports full per-account resource isolation out of the box. Resources created by one account are invisible to all others — no configuration flag required.

## How It Works

Every incoming request carries an AWS SigV4 `Authorization` header. Mimir reads the **Access Key ID** (AKID) from that header and applies one simple rule:

> **If the AKID is exactly 12 digits, it is used as the account ID.**  
> Any other key format (e.g. `AKIAIOSFODNN7EXAMPLE`) falls back to `MIMIR_DEFAULT_ACCOUNT_ID`.
...
```bash
MIMIR_DEFAULT_ACCOUNT_ID=000000000000   # default
```
...
```bash
MIMIR_DEFAULT_ACCOUNT_ID=123456789012
```

## Enabling Multi-Account Isolation

Use 12-digit numeric access key IDs. The secret access key can be any non-empty string — Mimir does not validate signatures by default.
...
Mimir embeds the resolved account ID in every ARN it generates:
...
All services that use `StorageFactory` participate in account isolation automatically. This covers every service in Mimir — SQS, SNS, S3, DynamoDB, Lambda, SSM, Secrets Manager, KMS, Kinesis, EventBridge, Cognito, RDS, ElastiCache, OpenSearch, MSK, and more.
...
By default Mimir **does not** validate SigV4 signatures — only the access key ID matters for account resolution. The secret access key can be any non-empty string.

To enforce real signature validation:

```bash
MIMIR_AUTH_VALIDATE_SIGNATURES=true
MIMIR_AUTH_PRESIGN_SECRET=your-secret   # for pre-signed URL verification
```
...
Storage keys are namespaced per account at the persistence layer. When using `persistent`, `hybrid`, or `wal` storage modes, each account's data is stored under its own key prefix. Restarting Mimir restores each account's resources independently.

## Configuration Reference

| Variable | Default | Description |
|---|---|---|
| `MIMIR_DEFAULT_ACCOUNT_ID` | `000000000000` | Account ID used when the AKID is not exactly 12 digits |
| `MIMIR_DEFAULT_REGION` | `us-east-1` | Region used when not derivable from the `Authorization` header |
| `MIMIR_AUTH_VALIDATE_SIGNATURES` | `false` | Enforce SigV4 signature verification |
