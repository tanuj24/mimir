...
Mimir publishes images to [Docker Hub (`mimir/mimir`)](https://hub.docker.com/r/mimir/mimir).
...
|---|---|---|
| **Standard** | Mimir native binary only | General use — CI, local dev, Testcontainers **(recommended)** |
| **Compat** | Mimir + Python 3 + AWS CLI + boto3 | Workflows that need AWS tooling available inside the container |
...
The AWS CLI is pre-configured to talk to the local Mimir endpoint — no `--endpoint-url` flag is needed in hook scripts:
