...
Review pull requests in the Mimir repository with AWS compatibility as the primary concern.

Mimir is a Java-based local AWS emulator built on Quarkus. Its goal is to match AWS SDK and AWS CLI behavior through real AWS wire protocols, not convenience APIs or custom abstractions.
...
3. Reuse existing Mimir patterns
...
- New service patterns where an existing Mimir pattern should be reused
- Direct storage implementation usage instead of `StorageFactory`

## Architecture Expectations

Mimir follows a layered design:
...
Mimir implements real AWS wire protocols. Review protocol-affecting changes carefully.
...
- Does it reuse an existing Mimir pattern?
...
- `medium`: inconsistent with Mimir architecture, wiring, or testing expectations
- `low`: maintainability, clarity, or minor convention issue

Prefer comments that explain:

- what is risky
- why it matters in Mimir
...
2. Then existing Mimir behavior
