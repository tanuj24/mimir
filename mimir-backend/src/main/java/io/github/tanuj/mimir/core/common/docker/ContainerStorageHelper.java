 ...
 * <p>Two modes:
 * <ul>
 *   <li>Named-volume (default) — Mimir manages per-resource Docker named volumes labelled
 *       {@code mimir=true}. Active when {@code MIMIR_STORAGE_HOST_PERSISTENT_PATH} is not set.</li>
 *   <li>Host-path (legacy) — active when {@code MIMIR_STORAGE_HOST_PERSISTENT_PATH} is set;
...
public final class ContainerStorageHelper {
    ...
    public static String resourceName(String service, String volumeId, String fallbackId) {
        return "mimir-" + service + "-" + (volumeId != null ? volumeId : fallbackId);
    }

    /**
     * Returns {@code true} when named-volume mode is active.
     * Returns {@code false} only when {@code MIMIR_STORAGE_HOST_PERSISTENT_PATH} is set to
