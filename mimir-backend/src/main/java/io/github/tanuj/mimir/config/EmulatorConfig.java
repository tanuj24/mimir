...
public interface EmulatorConfig {
     ...
     *
     * Example: MIMIR_HOSTNAME=mimir makes SQS return
     * http://mimir:4566/000000000000/my-queue instead of http://localhost:4566/...
     *
    ...
    interface DnsConfig {
        /**
         * Additional hostname suffixes the embedded DNS server will resolve to Mimir's
         * container IP, alongside the primary {@code mimir.hostname}.
         *
         ...
         * appended after Mimir's embedded DNS to every spawned container's {@code HostConfig.Dns}.
         * This gives Lambda/CodeBuild/etc. a real secondary resolver so public hostnames still
         * resolve if Mimir's embedded forwarder cannot answer — mirroring the
         * {@code docker run --dns <MimirIP> --dns 8.8.8.8} workaround.
         *
         * <p>Disable (via {@code MIMIR_DNS_CONTAINER_FALLBACK_ENABLED=false}) in offline or
        ...
        /**
         * Ordered list of public DNS resolvers used both as the fallback upstream for Mimir's
         ...
         * <pre>
         * MIMIR_DNS_CONTAINER_FALLBACK_SERVERS=1.1.1.1,1.0.0.1
         * </pre>
         */
    ...
    interface StorageConfig {
        ...
        @WithDefault("${mimir.storage.persistent-path}")
    ...
    interface DuckConfig {
        /** When set, Mimir uses this URL and skips mimir-duck container management. */
        Optional<String> url();

        @WithDefault("mimir/mimir-duck:latest")
        String defaultImage();
    }
    ...
    interface OpenSearchServiceConfig {
        ...
        @WithDefault("${mimir.storage.persistent-path}/opensearch")
        String dataPath();
    ...
    interface BcmDataExportsServiceConfig {
        ...
        /**
         * Same semantics as {@code mimir.services.cur.emit-mode} but applied to
    ...
    interface EcrServiceConfig {
        ...
        @WithDefault("mimir-ecr-registry")
        ...
        @WithDefault("${mimir.storage.persistent-path}/ecr")
        String dataPath();
    ...
    interface LambdaServiceConfig {
         ...
         * Mimir mirrors that semantics and partitions counters by the region
         ...
         * When absent, Mimir injects credentials from its own environment
         * (AWS_ACCESS_KEY_ID, etc.) or falls back to test/test/test.
         * Blank values are treated as absent.
         *
         * Env var: MIMIR_SERVICES_LAMBDA_AWS_CONFIG_PATH
         */
        ...
        interface HotReload {
             ...
             *
             * Env var: MIMIR_SERVICES_LAMBDA_HOT_RELOAD_ENABLED
             */
            @WithDefault("false")
            boolean enabled();
             ...
             *
             * Env var: MIMIR_SERVICES_LAMBDA_HOT_RELOAD_ALLOWED_PATHS
             */
    ...
    interface Ec2ServiceConfig {
        ...
        /** Port on the Mimir host for the IMDS HTTP server (169.254.169.254 equivalent). */
        @WithDefault("9169")
        int imdsPort();
    ...
    interface EksServiceConfig {
         ...
         *   <li>{@code network} — the container DNS name {@code https://mimir-eks-<name>:6443},
         *       reachable from other containers on the Docker network (pre-#1118 behaviour). Falls
         *       back to the host endpoint when Mimir runs natively.</li>
         * </ul>
         */
        @WithDefault("host")
        String endpointMode();
         ...
         * produced by {@code aws eks get-token} is validated by Mimir and mapped to cluster-admin.
    ...
    /**
     * Optional TLS configuration for enabling HTTPS on the Mimir server.
     ...
     */
    interface TlsConfig {
        /** Enable TLS/HTTPS on the server. Env: MIMIR_TLS_ENABLED */
        @WithDefault("false")
        boolean enabled();

        /** Path to PEM certificate file. Env: MIMIR_TLS_CERT_PATH */
        Optional<String> certPath();

        /** Path to PEM private key file. Env: MIMIR_TLS_KEY_PATH */
         ...
         * and reused across restarts. Env: MIMIR_TLS_SELF_SIGNED
         */
        @WithDefault("true")
        boolean selfSigned();
    }
    ...
    interface DockerConfig {
         ...
         * When set, overrides the system default. Useful when Mimir runs inside Docker
