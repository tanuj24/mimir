...
class DynamoDbServiceTest {
    ...
    @Test
    void updateExpressionAllowsMetadataAttributeName() {
        String region = "eu-west-1";
        // Regression for floci-io/floci#1169: "metadata" is not in the
        // current DynamoDB reserved-word list and should work as a bare
        // UpdateExpression path.
        createUsersTable(region);

        ObjectNode initialItem = mapper.createObjectNode();
        initialItem.set("userId", attributeValue("S", "u13"));
        service.putItem("Users", initialItem, region);

        ObjectNode values = mapper.createObjectNode();
        values.set(":v", attributeValue("S", "profile"));

        service.updateItem("Users", userIdKey("u13"), null,
                "SET metadata = :v", null, values, "ALL_NEW", region);

        JsonNode stored = service.getItem("Users", userIdKey("u13"), region);
        assertEquals("profile", stored.get("metadata").get("S").asText());
    }
