 ...
 * - Includes custom hostnames from MIMIR_HOSTNAME and MIMIR_BASE_URL in certificate SANs
...
class TlsCertificateHostnameTest {
    ...
    void cleanupSystemProperties() {
        ...
        System.clearProperty("mimir.tls.cert-path");
        System.clearProperty("mimir.tls.key-path");
    ...
    @Test
    void testMimirHostnameIncludedInCertificateSans() throws Exception {
        // Arrange
        ...
        assertTrue(sans.contains("mimir"), 
            "Certificate SANs should contain 'mimir' from MIMIR_HOSTNAME. Found: " + sans);
    ...
    void testBaseUrlHostnameIncludedInCertificateSans() throws Exception {
        ...
        assertTrue(sans.contains("myhost"), 
            "Certificate SANs should contain 'myhost' from MIMIR_BASE_URL. Found: " + sans);
    }
    ...
    void testBaseUrlIpAddressIncludedInCertificateSans() throws Exception {
        ...
        assertTrue(sans.contains("192.168.1.100"), 
            "Certificate SANs should contain IP address from MIMIR_BASE_URL. Found: " + sans);
    }
    ...
    void testBothHostnamesIncludedInCertificateSans() throws Exception {
        ...
        assertTrue(sans.contains("newhost"), 
            "Certificate SANs should contain 'newhost' from MIMIR_HOSTNAME. Found: " + sans);
        assertTrue(sans.contains("oldhost"), 
            "Certificate SANs should contain 'oldhost' from MIMIR_BASE_URL. Found: " + sans);
    }
    ...
    void testMissingMetadataTriggersRegeneration() throws Exception {
        ...
        Path keyFile = tlsDir.resolve("mimir-selfsigned.key");
        ...
        CertificateGenerator.GeneratedCertificate generated = gen.generateCertificate(
            "localhost", 
            List.of("localhost", "127.0.0.1", "0.0.0.0", "*.localhost", "localhost.mimir.local", "*.localhost.mimir.local"), 
        ...
        List<String> sans = extractSansFromCertificate(cert);
        assertTrue(sans.contains("mimir"), 
            "Regenerated certificate should contain 'mimir'. Found: " + sans);
    }
    ...
    void testUserProvidedCertificatesUsedWithoutModification() throws Exception {
        ...
        System.setProperty("mimir.tls.enabled", "true");
        System.setProperty("mimir.tls.cert-path", userCertFile.toString());
        System.setProperty("mimir.tls.key-path", userKeyFile.toString());
        System.setProperty("mimir.storage.persistent-path", tempDir.toString());
        ...
        Path selfSignedCert = tempDir.resolve("tls/mimir-selfsigned.crt");
    ...
    void testTlsDisabledSkipsCertificateGeneration() throws Exception {
        ...
        if (Files.exists(tlsDir)) {
            assertFalse(Files.exists(tlsDir.resolve("mimir-selfsigned.crt")),
    ...
    void testUnchangedConfigurationReusesCertificate() throws Exception {
        ...
        String initialMetadata = Files.readString(tempDir.resolve("tls/mimir-selfsigned.metadata.json"));
        ...
        String newMetadata = Files.readString(tempDir.resolve("tls/mimir-selfsigned.metadata.json"));
    ...
    static Stream<Map<String, String>> defaultConfigurations() {
        return Stream.of(
            Map.of("mimir.tls.enabled", "true", "mimir.tls.self-signed", "true"),
            Map.of("mimir.tls.enabled", "true", "mimir.tls.self-signed", "true", 
                   "mimir.base-url", "http://localhost:4566"),
            Map.of("mimir.tls.enabled", "true", "mimir.tls.self-signed", "true", 
                   "mimir.base-url", "http://127.0.0.1:4566"),
            Map.of("mimir.tls.enabled", "true", "mimir.tls.self-signed", "true", 
                   "mimir.hostname", "localhost"),
            Map.of("mimir.tls.enabled", "true", "mimir.tls.self-signed", "true", 
                   "mimir.base-url", "https://localhost:4566"),
            Map.of("mimir.tls.enabled", "true", "mimir.tls.self-signed", "true", 
                   "mimir.base-url", "http://0.0.0.0:4566")
        );
    }
}
