...
resource "aws_sqs_queue" "jobs" {
  name                       = "mimir-compat-jobs"
  visibility_timeout_seconds = 30
  message_retention_seconds  = 86400
}

resource "aws_sqs_queue" "jobs_dlq" {
  name = "mimir-compat-jobs-dlq"
}
...
resource "aws_iam_role" "lambda_exec" {
  name               = "mimir-compat-lambda-exec"
  assume_role_policy = data.aws_iam_policy_document.lambda_assume_role.json
}

# -- SSM Parameters ------------------------------------------------------------
resource "aws_ssm_parameter" "db_url" {
  name  = "/mimir-compat/db-url"
  type  = "String"
  value = "jdbc:postgresql://localhost:5432/app"
}

resource "aws_ssm_parameter" "api_key" {
  name  = "/mimir-compat/api-key"
  type  = "SecureString"
...
resource "aws_cloudwatch_metric_alarm" "cpu" {
  alarm_name          = "mimir-compat-cpu-alarm"
...
resource "aws_route53_record" "app" {
  ...
  name    = "app.mimir-compat.internal"
  type    = "A"
  ttl     = 300
  records = ["10.0.1.10"]
}
...
resource "aws_route53_health_check" "app" {
  fqdn              = "app.mimir-compat.internal"
  port              = 80
  type              = "HTTP"
