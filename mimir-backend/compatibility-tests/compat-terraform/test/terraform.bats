#!/usr/bin/env bats
# Terraform Compatibility Tests for mimir
...
setup_file() {
    ...
    run terraform init -backend-config=/tmp/mimir-backend.hcl \
        -var="endpoint=${MIMIR_ENDPOINT}" -input=false -no-color
    if [ "$status" -ne 0 ]; then
...
teardown_file() {
    ...
    terraform destroy -var="endpoint=${MIMIR_ENDPOINT}" -input=false -auto-approve -no-color || true
}
...
@test "Terraform: S3 bucket created" {
    run aws_cmd s3api head-bucket --bucket mimir-compat-app
    assert_success
}

@test "Terraform: SQS queue created" {
    run aws_cmd sqs get-queue-url --queue-name mimir-compat-jobs
    assert_success
...
@test "Terraform: SNS topic created" {
    ...
    assert_success
    assert_output --partial "mimir-compat-events"
}

@test "Terraform: DynamoDB table created" {
    run aws_cmd dynamodb describe-table --table-name mimir-compat-items
    assert_success
    assert_output --partial "ACTIVE"
}

@test "Terraform: SSM parameter created" {
    run aws_cmd ssm get-parameter --name /mimir-compat/db-url
    assert_success
    assert_output --partial "jdbc:"
}

@test "Terraform: Secrets Manager secret created" {
    run aws_cmd secretsmanager describe-secret --secret-id "mimir-compat/db-creds"
    assert_success
    assert_output --partial "mimir-compat"
}

@test "Terraform: RDS DB instance created and available" {
    run aws_cmd rds describe-db-instances --db-instance-identifier mimir-compat-db
    assert_success
    assert_output --partial "mimir-compat-db"
    assert_output --partial "available"
}

@test "Terraform: CloudWatch alarm created with tags" {
    run aws_cmd cloudwatch describe-alarms --alarm-names mimir-compat-cpu-alarm
    assert_success
    assert_output --partial "mimir-compat-cpu-alarm"

    ALARM_ARN=$(aws_cmd cloudwatch describe-alarms --alarm-names mimir-compat-cpu-alarm \
...
@test "Terraform: VPC created with custom DNS settings" {
    run aws_cmd ec2 describe-vpcs \
        --filters "Name=tag:Name,Values=mimir-compat-vpc"
    assert_success
    assert_output --partial "mimir-compat-vpc"
...
@test "Terraform: Route53 hosted zone created" {
    ZONE_ID=$(aws_cmd route53 list-hosted-zones \
        --query "HostedZones[?Name=='mimir-compat.internal.'].Id | [0]" \
        --output text | sed 's|/hostedzone/||')
    [ -n "$ZONE_ID" ]
    run aws_cmd route53 get-hosted-zone --id "$ZONE_ID"
    assert_success
    assert_output --partial "mimir-compat.internal"
}
...
@test "Terraform: Route53 A record created" {
    ...
    assert_success
    assert_output --partial "app.mimir-compat.internal"
...
@test "Terraform: Route53 health check created" {
    HEALTH_CHECK_ID=$(aws_cmd route53 list-health-checks \
        --query "HealthChecks[?HealthCheckConfig.FullyQualifiedDomainName=='app.mimir-compat.internal'].Id | [0]" \
        --output text)
    [ -n "$HEALTH_CHECK_ID" ]
