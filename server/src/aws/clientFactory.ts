 ...
 * Every client is pointed at the the Mimir backend endpoint with dummy credentials.
 */
...
export interface ClientOptions {
  ...
  forcePathStyle?: boolean;
  /** Override the the Mimir backend endpoint (e.g. the browser-facing one for presigning). */
  endpoint?: string;
}

/**
 * Generic factory: pass any AWS SDK v3 client class and get an instance
 * wired to the Mimir backend. Keeps every service route module a one-liner.
 *
 *   const s3 = makeClient(S3Client, { forcePathStyle: true });
 */
export function makeClient<T>(
  ClientCtor: new (cfg: BackendClientConfig) => T,
  opts: ClientOptions = {},
): T {
  return new ClientCtor({
    region: opts.region ?? config.region,
    endpoint: opts.endpoint ?? config.backendEndpoint,
    credentials: {
